...
describe('validateFormParams', () => {
  ...
  it('should throw an error if formUrlParameters are not provided', () => {
    expect(() => validateFormParams(undefined as unknown as Parameters<typeof validateFormParams>[0])).toThrow(
      'Form url parameters not provided',
    );
  });
  ...
  it('should throw an error if one of the form parameters is invalid', () => {
    ...
    expect(() => validateFormParams(invalidParameters)).toThrow('One of form parameter is invalid: language');
  });

  it('should throw an error if form code is invalid', () => {
    const invalidFormParameters = { ...mockFormUrlParameters, form: 'invalidFormCode' };
    expect(() => validateFormParams(invalidFormParameters)).toThrow('Form code not allowed: invalidFormCode');
  });
  ...
  it('should throw an error if language is not 2 characters', () => {
    ...
    expect(() => validateFormParams(invalidLanguageParameters)).toThrow('Form language not allowed: eng');
  });
});
