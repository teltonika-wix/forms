...
describe('tailwindPreset', () => {
  it('should check the equality of Tailwind presets', () => {
    ...
    const defaultEditorOptions = 'default' in tailwindPresetEditor ? tailwindPresetEditor.default : {};

    expect({ ...defaultOptions, plugins: null }).toMatchObject({ ...(defaultEditorOptions as object), plugins: null });
  });
});
